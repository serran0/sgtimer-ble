...
function resetInactivityTimer() {
  ...
  inactivityTimer = setTimeout(() => location.reload(), 60000);
}
...
ws.onmessage = (e) => {
  ...
  // Always-immediate: device/connection state, server lifecycle, settings
  const immediateTypes = [
    "DEVICE_CONNECTED", "DEVICE_DISCONNECTED", "WATCHDOG",
    "SERVER_HELLO", "RELOAD", "SETTINGS_UPDATE", "SESSION_SYNC", "LENS_CHANGED"
  ];
...
function handleMessage(msg) {
  ...
  if (msg.type === "LENS_CHANGED") return; // display page ignores lens changes
