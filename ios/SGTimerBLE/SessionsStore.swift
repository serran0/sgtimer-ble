...
class SessionsStore {
    ...
    init() {
        ...
        archiveRoot = dataDir.appendingPathComponent("archive")
    ...
    func clearSessions() -> (moved: Int, timestamp: String) {
        let fmt = DateFormatter()
        fmt.dateFormat = "yyyy-MM-dd_HH-mm"
        let timestamp = fmt.string(from: Date())
        let targetDir = archiveRoot.appendingPathComponent(timestamp)
        try? FileManager.default.createDirectory(at: targetDir, withIntermediateDirectories: true)

        guard let files = try? FileManager.default.contentsOfDirectory(
            at: dataDir, includingPropertiesForKeys: nil
        ) else { return (0, timestamp) }
